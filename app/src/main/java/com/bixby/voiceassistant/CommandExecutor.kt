...
class CommandExecutor(private val context: Context) {
    ...
    fun executeIfSupported(rawText: String): Result {
        ...
        val isHardwareOrSystemCommand = command.contains("torch") || command.contains("flashlight") ||
            command.contains("bluetooth") || command.contains("wifi") || command.contains("wi-fi") ||
            command.contains("go home") || command == "home" || command.contains("go back") || command == "back" ||
            command.contains("open settings") || command.startsWith("open ") || command.startsWith("launch ") ||
            command.startsWith("start ") || command.contains("call") || command.contains("dial") ||
        ...
        if (command.contains("torch") || command.contains("flashlight")) {
            val enabled = when { command.contains("turn off") || command.contains("switch off") -> false; command.contains("turn on") || command.contains("switch on") -> true; else -> !torchState }
        ...
        if (command.contains("wifi") || command.contains("wi-fi")) {
            val on = command.contains("turn on") || command.contains("switch on")
            val off = command.contains("turn off") || command.contains("switch off")
            if (on || off) return Result.Handled(handleWifiToggle(on))
            openSettings(Settings.ACTION_WIFI_SETTINGS); return Result.Handled("Opening Wi-Fi settings.")
        }
        ...
        if (command.startsWith("open ") || command.startsWith("launch ") || command.startsWith("start ")) {
            if (!command.contains("settings") && !command.contains("wifi") && !command.contains("wi-fi")) return Result.Handled(launchApp(command))
        }
    ...
    private fun handleMessage(command: String): String {
        ...
        val targetAndBody = Regex("^(?:to\\s+)?(.+?)(?:\\s+(?:saying|that says|says|:|message)\\s+)(.+)$").find(withoutPrefix)
        val target = targetAndBody?.groupValues?.get(1)?.trim() ?: return "Tell me the contact and message, for example: message Rahul saying hello."
        ...
        return try {
            ...
            if (whatsapp && context.packageManager.resolveActivity(intent, PackageManager.MATCH_DEFAULT_ONLY) == null) return "WhatsApp is not installed."
            context.startActivity(intent)
            if (whatsapp) "Opening WhatsApp to message $target." else "Opening messages to $target."
        } catch (_: Exception) { "I couldn't open messaging right now." }
    }
    ...
    private fun resolveContactNumber(name: String): String? {
        ...
        return try { val p = arrayOf(android.provider.ContactsContract.CommonDataKinds.Phone.NUMBER); context.contentResolver.query(android.provider.ContactsContract.CommonDataKinds.Phone.CONTENT_URI, p, "${android.provider.ContactsContract.CommonDataKinds.Phone.DISPLAY_NAME} LIKE ?", arrayOf("%$name%"), null)?.use { c -> if (c.moveToFirst()) c.getString(0) else null } } catch (_: Exception) { null }
    }
    ...
    private fun launchApp(command: String): String { val requested = command.removePrefix("open ").removePrefix("launch ").removePrefix("start ").removePrefix("the ").trim(); if (requested.isEmpty()) return "Please tell me which app to open."; val launcher = Intent(Intent.ACTION_MAIN).addCategory(Intent.CATEGORY_LAUNCHER); val matches = context.packageManager.queryIntentActivities(launcher, PackageManager.MATCH_ALL); val match = matches.firstOrNull { it.activityInfo.packageName != context.packageName && it.loadLabel(context.packageManager).toString().trim().lowercase() == requested } ?: matches.firstOrNull { it.activityInfo.packageName != context.packageName && it.loadLabel(context.packageManager).toString().trim().lowercase().contains(requested) }; val intent = match?.activityInfo?.let { Intent(Intent.ACTION_MAIN).addCategory(Intent.CATEGORY_LAUNCHER).setClassName(it.packageName, it.name).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK) }; return if (intent != null) { context.startActivity(intent); "Opening ${match.loadLabel(context.packageManager)}." } else "I couldn't find that app." }

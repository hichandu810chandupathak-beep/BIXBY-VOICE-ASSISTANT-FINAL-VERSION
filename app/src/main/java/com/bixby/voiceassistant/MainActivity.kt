...
class MainActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!getSharedPreferences("bixby_onboarding", Context.MODE_PRIVATE).getBoolean("completed", false)) {
        ...
        applySystemBarInsets(); startOrbPulse(); textToSpeech = TextToSpeech(this, this)
        if (hasRecordAudioPermission()) setupSpeechRecognizer() else status.text = "Microphone permission needed"
        ...
        if (intent.getBooleanExtra("HOTWORD_TRIGGERED", false)) requestPermissionsAndListen()
    }
    ...
    private fun requestPermissionsAndListen() {
        val needed = buildList { if (!hasRecordAudioPermission()) add(Manifest.permission.RECORD_AUDIO); if (ContextCompat.checkSelfPermission(this@MainActivity, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) add(Manifest.permission.CAMERA); if (android.os.Build.VERSION.SDK_INT >= 31 && ContextCompat.checkSelfPermission(this@MainActivity, Manifest.permission.BLUETOOTH_CONNECT) != PackageManager.PERMISSION_GRANTED) add(Manifest.permission.BLUETOOTH_CONNECT) }
        if (needed.isNotEmpty()) ActivityCompat.requestPermissions(this, needed.toTypedArray(), REQUEST_PERMISSIONS) else if (speechRecognizer == null) { try { setupSpeechRecognizer() } catch (e: Exception) { android.util.Log.e("BixbyLifecycle", "SpeechRecognizer initialization failed", e); status.text = "Speech recognition unavailable" } ; startListening() } else startListening()
    }
    ...
    private fun setupSpeechRecognizer() {
        ...
        try {
            if (!hasRecordAudioPermission() || !SpeechRecognizer.isRecognitionAvailable(this)) { status.text = "Speech recognition unavailable"; return }
            speechRecognizer = SpeechRecognizer.createSpeechRecognizer(this).apply { setRecognitionListener(object : RecognitionListener { override fun onReadyForSpeech(params: Bundle?) { isListening = true; status.text = "Listening..." }; override fun onBeginningOfSpeech() { status.text = "Listening..." }; override fun onRmsChanged(rmsdB: Float) = Unit; override fun onBufferReceived(buffer: ByteArray?) = Unit; override fun onEndOfSpeech() { isListening = false; status.text = "Thinking..." }; override fun onPartialResults(partialResults: Bundle?) { partialResults?.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION)?.firstOrNull()?.let { status.text = it } }; override fun onResults(results: Bundle?) { isListening = false; val text = results?.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION)?.firstOrNull(); if (text.isNullOrBlank()) { status.text = "I didn't catch that"; return }; status.text = "Thinking..."; handleRecognizedText(text) }; override fun onError(error: Int) { isListening = false; status.text = if (error == SpeechRecognizer.ERROR_NO_MATCH || error == SpeechRecognizer.ERROR_SPEECH_TIMEOUT) "Didn't catch that" else "Try again" }; override fun onEvent(eventType: Int, params: Bundle?) = Unit }) }
        } catch (e: Exception) { android.util.Log.e("BixbyLifecycle", "SpeechRecognizer initialization failed", e); speechRecognizer = null; status.text = "Speech recognition unavailable" }
    }
    private fun startListening() { if (!hasRecordAudioPermission()) { status.text = "Microphone permission needed"; return }; if (speechRecognizer == null) { try { setupSpeechRecognizer() } catch (e: Exception) { android.util.Log.e("BixbyLifecycle", "SpeechRecognizer setup failed", e); return } }; hideKeyboardInput(); val recognizer = speechRecognizer ?: return; val intent = Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH).apply { putExtra(RecognizerIntent.EXTRA_LANGUAGE_MODEL, RecognizerIntent.LANGUAGE_MODEL_FREE_FORM); putExtra(RecognizerIntent.EXTRA_LANGUAGE, "hi-IN"); putExtra(RecognizerIntent.EXTRA_LANGUAGE_PREFERENCE, "hi-IN"); putExtra(RecognizerIntent.EXTRA_PARTIAL_RESULTS, true); putExtra(RecognizerIntent.EXTRA_MAX_RESULTS, 3) }; status.text = "Listening..."; isListening = true; try { recognizer.startListening(intent) } catch (e: Exception) { android.util.Log.e("BixbyLifecycle", "SpeechRecognizer start failed", e); isListening = false; status.text = "Try again" } }
    private fun stopListening() { try { speechRecognizer?.cancel() } catch (e: Exception) { android.util.Log.e("BixbyLifecycle", "SpeechRecognizer cancel failed", e) }; isListening = false; status.text = "Listening..." }
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) { super.onRequestPermissionsResult(requestCode, permissions, grantResults); if (requestCode == REQUEST_PERMISSIONS) { if (hasRecordAudioPermission()) { try { if (speechRecognizer == null) setupSpeechRecognizer(); startListening() } catch (e: Exception) { android.util.Log.e("BixbyLifecycle", "Post-permission speech setup failed", e); status.text = "Speech recognition unavailable" } } else status.text = "Microphone permission needed" } }
    override fun onDestroy() { typingRunnable?.let(mainHandler::removeCallbacks); try { speechRecognizer?.destroy() } catch (e: Exception) { android.util.Log.e("BixbyLifecycle", "SpeechRecognizer destroy failed", e) }; speechRecognizer = null; try { textToSpeech?.stop(); textToSpeech?.shutdown() } catch (e: Exception) { android.util.Log.e("BixbyLifecycle", "TTS shutdown failed", e) }; textToSpeech = null; stopOrbPulse(); mainHandler.removeCallbacksAndMessages(null); super.onDestroy() }
}

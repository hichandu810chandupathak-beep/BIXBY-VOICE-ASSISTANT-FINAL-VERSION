...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        homeContent = findViewById(R.id.homeContent); listeningContent = findViewById(R.id.listeningContent); conversationContent = findViewById(R.id.conversationContent)
        userBubble = findViewById(R.id.userBubble); assistantBubble = findViewById(R.id.assistantBubble); weatherCard = findViewById(R.id.weatherCard)
        ...
        findViewById<ImageButton>(R.id.micButton).setOnClickListener { if (ignoreNextMicTap) { ignoreNextMicTap=false; return@setOnClickListener }; if (voiceTurnInProgress) return@setOnClickListener; if (listeningByButton) stopListening() else listen() }
    ...
    private fun selectMaleVoice(locale:Locale){
        try{
            val candidates:List<Voice> = tts.voices.orEmpty().filter{it.locale.language==locale.language}.sortedWith(compareByDescending<Voice>{val n=it.name.lowercase(Locale.ROOT);if(n.contains("male")||n.contains("man")||n.contains("masculine"))1 else 0}.thenByDescending{it.locale==locale})

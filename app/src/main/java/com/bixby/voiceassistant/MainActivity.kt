...
class MainActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    private fun startVoiceAfterBackgroundSetup() {
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            if (!micGranted || !notificationGranted) { withContext(Dispatchers.Main) { status.text = "Microphone permission needed" }; return@launch }
            withContext(Dispatchers.Main) {
                try {
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) { super.onRequestPermissionsResult(requestCode, permissions, grantResults); if (requestCode == REQUEST_PERMISSIONS) { if (hasRecordAudioPermission() && hasNotificationPermission()) startVoiceAfterBackgroundSetup() else status.text = "Microphone permission needed" } }

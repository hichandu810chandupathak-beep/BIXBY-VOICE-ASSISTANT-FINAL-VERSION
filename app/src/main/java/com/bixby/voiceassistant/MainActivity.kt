...
class MainActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    private fun requestPermissionsAndListen() {
        ...
        if (needed.isNotEmpty()) ActivityCompat.requestPermissions(this, needed.toTypedArray(), REQUEST_PERMISSIONS) else startManualVoice()
    }
    private fun startManualVoice() {
        if (!hasRecordAudioPermission() || !hasNotificationPermission()) { status.text = "Microphone permission needed"; return }
        try {
            if (speechRecognizer == null) setupSpeechRecognizer()
            startListening()
            HotwordListeningService.start(this)
        } catch (e: Exception) {
            android.util.Log.e("BixbyCrash", "Manual voice trigger failed", e)
    ...
    private fun setupSpeechRecognizer() {
        ...
        } catch (e: Exception) { android.util.Log.e("BixbyCrash", "SpeechRecognizer initialization failed", e); speechRecognizer = null; status.text = "Speech recognition unavailable" }
    }
    ...
    private fun stopListening() { try { speechRecognizer?.cancel() } catch (e: Exception) { android.util.Log.e("BixbyCrash", "SpeechRecognizer cancel failed", e) }; isListening = false; status.text = "Listening..." }
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) { super.onRequestPermissionsResult(requestCode, permissions, grantResults); if (requestCode == REQUEST_PERMISSIONS) { if (hasRecordAudioPermission() && hasNotificationPermission()) startManualVoice() else status.text = "Microphone permission needed" } }
    override fun onDestroy() { typingRunnable?.let(mainHandler::removeCallbacks); try { speechRecognizer?.destroy() } catch (e: Exception) { android.util.Log.e("BixbyCrash", "SpeechRecognizer destroy failed", e) }; speechRecognizer = null; try { textToSpeech?.stop(); textToSpeech?.shutdown() } catch (e: Exception) { android.util.Log.e("BixbyCrash", "TTS shutdown failed", e) }; textToSpeech = null; stopOrbPulse(); mainHandler.removeCallbacksAndMessages(null); super.onDestroy() }
}
